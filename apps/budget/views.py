...
def get_allocation_picker(request, allocation_id):
    ...
    for transaction in unallocated:
        ...
        if len(keywords) < 1 or not keywords[0]:
            break
    ...
    sorted_transactions = sorted(unallocated, key=lambda x: getattr(x, 'is_match', False), reverse=True)
    
    return render(request, 'budget/partials/allocation_modal.html', {
...
def save_allocations(request, allocation_id):
    ...
    if request.method == "POST":
        ...
        Transaction.objects.filter(
            id__in=selected_ids, 
            user=request.user
        ).update(cost_allocation=allocation)
